...
def readCameraData(file):
    ...
    def to_matrix(line):
        return np.array([float(x) for x in line.split(" ")[1:]]).reshape(4, 4)

    matrices = []

    for chunk in chunks:
        left_matrix = to_matrix(chunk[1])
        right_matrix = to_matrix(chunk[2])
        matrices.append({"left": left_matrix, "right": right_matrix})
    return matrices
