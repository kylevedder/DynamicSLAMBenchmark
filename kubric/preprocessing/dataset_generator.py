...
def reproject(coords, camera, camera_pos, num_frames, bbox=None):
  ...
  """
    ...
    if bbox is not None:
        ...
        local_to_world = tf.linalg.lstsq(tf.cast(coord_box, tf.float32),
                                         bbox_homo)
        world_coords = tf.matmul(
            tf.cast(tf.concat([coords, tf.ones_like(coords[:, 0:1])], axis=1),
...
def rotate_surface_normals(
    ...
    faces_away = tf.logical_and(faces_away,
                                tf.logical_not(faces_away_query > 0))
    return faces_away
...
def single_object_reproject(
  ...
  """
    ...
    obj_occ = tf.logical_or(obj_occ, tf.greater(obj_reproj[:, :, 1],
                                                window[2]))
    obj_occ = tf.logical_or(obj_occ, tf.greater(obj_reproj[:, :, 0],
                                                window[3]))
...
def get_num_to_sample(counts, max_seg_id, max_sampled_frac, tracks_to_sample):
  ...
  """
    ...
    def scan_fn(prev_output, count_seg):
        ...
        want_to_sample = (tf.cast(remaining_needed, tf.float32) *
                          tf.cast(desired_frac, tf.float32))
        want_to_sample = tf.cast(tf.round(want_to_sample), tf.int32)
        max_to_sample = (tf.cast(count_seg, tf.float32) *
                         tf.cast(max_sampled_frac, tf.float32))
    ...
    num_to_sample = tf.concat(
        [
            num_to_sample,
            tf.zeros([max_seg_id - tf.shape(num_to_sample)[0]],
                     dtype=tf.int32),
        ],
        axis=0,
    )
...
def track_points(
  ...
  """
    ...
    def extract_box(x):
        x = x[start_vec[0]::sampling_stride,
    ...
    cnt = tf.math.bincount(
        tf.cast(tf.reshape(segmentations_box, [-1]), tf.int32))
    ...
    for i in [0, 2]:
        for j in [0, 2]:
            diff = sn_pad[:, i:shp[1] + i,
                          j:shp[2] + j, :] - surface_normal_map
    ...
    for i in range(max_seg_id):
        ...
        normals = tf.boolean_mask(tf.reshape(surface_normals_box, [-1, 3]),
                                  mask)
        trust_sn_mask = tf.boolean_mask(tf.reshape(trust_sn_box, [-1, 1]),
                                        mask)
        idx = tf.cond(
            tf.shape(pt)[0] > 0,
        ...
        if obj_id == -1:
            ...
            for fr in range(num_frames):
                ...
                pt_3d.append(
                    unproject(pt_coords_chunk[:, 1:], get_camera(fr),
                              depth_map[fr]))
            ...
            chosen_points.append(
                tf.cast(pt_coords, tf.float32) + pixel_to_raster)
            # if obj_id>num_objects, then we won't have a box.  We also won't have
            # points, so just use a dummy to prevent tf from crashing.
            bbox = tf.cond(obj_id >= tf.shape(bboxes_3d)[0],
                           lambda: bboxes_3d[0, :],
                           lambda: bboxes_3d[obj_id, :])
            quat = tf.cond(obj_id >= tf.shape(obj_quat)[0],
                           lambda: obj_quat[0, :], lambda: obj_quat[obj_id, :])
    ...
    chosen_points = (chosen_points - wd[:, 0, :3]) / (wd[:, 0, 3:] -
                                                      wd[:, 0, :3])
...
def add_tracks(data,
  ...
  """
    ...
    if any([s % sampling_stride != 0 for s in shp[:-1]]):
        raise ValueError(
            'All video dims must be a multiple of sampling_stride.')
    ...
    query_points, target_points_3d, target_points, occluded, depth_map = track_points(
        ...
        data['camera']['focal_length'], data['camera']['positions'],
        data['camera']['quaternions'], data['camera']['sensor_width'],
        crop_window, tracks_to_sample, sampling_stride, max_seg_id,
        max_sampled_frac)
    video = data['video']
    ...
    depth_map = tf.image.crop_and_resize(
        depth_map,
        tf.cast(crop_window, tf.float32),
        tf.range(num_frames),
        train_size,
...
def create_point_tracking_dataset(train_size=(256, 256),
  ...
  """
    ...
    if repeat:
        ds = ds.repeat()
    print("Starting track extraction")
    ds = ds.map(functools.partial(add_tracks,
...
def plot_tracks(rgb, points, occluded, trackgroup=None):
    ...
    for i in range(rgb.shape[0]):
        fig = plt.figure(figsize=(256 / figure_dpi, 256 / figure_dpi),
                         dpi=figure_dpi,
                         frameon=False,
                         facecolor='w')
        ax = fig.add_subplot()
...
def save_sequence_as_pkl(save_dir: Path, idx: int, data):
    ...
    video_file = save_dir / f'{idx:04d}.mp4'
...
def main(save_directory: Path = Path() / "generated_data"):
    print(
        "Starting dataset generation by pulling movi_e data and extracting tracks"
    )
    ds = tfds.as_numpy(
