...
from scene_trajectory_benchmark.datasets import Argoverse2SceneFlow
from scene_trajectory_benchmark.eval import Evaluator
from scene_trajectory_benchmark.datastructures import QuerySceneSequence, GroundTruthParticleTrajectories, EstimatedParticleTrajectories, RawSceneItem
from scene_trajectory_benchmark.datastructures import PointCloud, O3DVisualizer
...
def perform_evaluate(mask: pd.DataFrame, result: pd.DataFrame,
    ...
    ) == 2, f"Query sequence length is not 2 as expected for Scene Flow; it's {len(query.scene_sequence)}"
    ...
    lookup[masked_array_idxes] = (stacked_points[masked_array_idxes], [0, 1],
                                  np.zeros((masked_array_idxes.shape[0], 2),
                                           dtype=bool))
...
def process_problem(
    ...
    def timestamp_to_dataset_idx(timestamp):
        ...
        return dataset._av2_sequence_id_and_timestamp_to_idx(
            sequence_name, timestamp), mask_data, result_data
...
def build_process_problems(
    ...
    assert mask_key_set == result_key_set, f"Mask and result keys do not match. Mask has {mask_key_set - result_key_set} more keys, Result has {result_key_set - mask_key_set} more keys."
...
def main_loop(mask_zip: Path, result_zip: Path, dataset: Argoverse2SceneFlow,
    ...
    mask_sequence_map, result_sequence_map = multiprocessor(
        load_feather_files, [mask_zip, result_zip],
        leave=False,
    ...
    evaluators: List[Evaluator] = multiprocessor(process_problem,
                                                 problems,
                                                 desc="Problems")
    sum(evaluators, start=Evaluator()).compute_results()
...
def build_multiprocessor(cpu_count: int):
    ...
    def single_threaded_process(
        ...
        return [
            worker(problem) for problem in tqdm.tqdm(problems,
                                                     disable=not verbose,
                                                     leave=leave,
                                                     desc=desc,
    ...
    def multi_threaded_process(
        ...
        with multiprocessing.Pool(cpus_to_use) as pool:
            return list(
                tqdm.tqdm(pool.imap(worker, problems),
                          total=len(problems),
                          disable=not verbose,
                          leave=leave,
                          bar_format=bar_format,
