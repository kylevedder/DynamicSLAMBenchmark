...
class BucketedEPEEvaluator(Evaluator):

    def __init__(self,
                 bucket_max_speed_meters_per_second: float = 3.0,
                 num_buckets: int = 50,
        ...
        self.bucket_edges = np.concatenate(
            np.linspace(0, bucket_max_speed_meters_per_second, num_buckets),
            [np.inf])
    ...
    @staticmethod
    def from_evaluator_list(evaluator_list: List["BucketedEPEEvaluator"]):
    ...
    def __add__(self, other: "BucketedEPEEvaluator"):
        if isinstance(other, int):
            if other == 0:
                return self

        assert isinstance(
            other, BucketedEPEEvaluator
        ), f"other must be a BucketedEPEEvaluator, got {type(other)}"
